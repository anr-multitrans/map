...
from MapTRV2Local.projects.mmdet3d_plugin.datasets.builder import build_dataloader
import argparse
import mmcv
import os
import shutil
            ...
            if vis_format == 'se_pts':
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_line_points, gt_labels_3d[0]):
                    ...
                    plt.quiver(x[:-1], y[:-1], x[1:] - x[:-1], y[1:] - y[:-1],
                               scale_units='xy', angles='xy', scale=1, color=colors_plt[gt_label_3d])
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_lines_bbox, gt_labels_3d[0]):
                    ...
                    # import pdb;pdb.set_trace()
                    plt.gca().add_patch(Rectangle(xy, width, height, linewidth=0.4,
                                                  edgecolor=colors_plt[gt_label_3d], facecolor='none'))
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_lines_fixed_num_pts, gt_labels_3d[0]):
                    ...
                    plt.plot(
                        x, y, color=colors_plt[gt_label_3d], linewidth=1, alpha=0.8, zorder=-1)
                    plt.scatter(
                ...
                gt_fixedpts_map_path = osp.join(
                    sample_dir, 'GT_fixednum_pts_MAP.png')
                plt.savefig(gt_fixedpts_map_path,
                ...
                gt_polyline_map_path = osp.join(
                    sample_dir, 'GT_polyline_pts_MAP.png')
                plt.savefig(gt_polyline_map_path,
                            bbox_inches='tight', format='png', dpi=1200)
                plt.close()
