...
from perturbation import PerturbedVectorizedLocalMap
...
def create_av2_infos_mp(root_path,
    ...
    for log_id in log_ids:
        result = get_data_from_logid(
            log_id, loader=loader, data_root=root_path, pc_range=pc_range)
...
def get_data_from_logid(log_id,
    ...
    for ts in cam_timestamps:
        ...
        map_anno, vector_map, patch_box = extract_local_map(
            avm, e2g_translation, e2g_rotation, pc_range)
        info["annotation"] = map_anno
        info["scene_token"] = "scene_1"
        
        # vis_path='/home/li/Documents/map/MapTRV2Local/tools/maptrv2/map_perturbation/visual/av2'
        # visual = RenderMap(info, vector_map, vis_path)
        # visual.vis_contours(map_anno, patch_box, 'annotation')
...
def extract_local_map(avm, e2g_translation, e2g_rotation, pc_range):
    ...
    ns = NuScenesMap("/home/li/Documents/map/data/sets/nuscenes")
    ns_exp = NuScenesMapExplorer(ns)
    pt = PerturbedVectorizedLocalMap(ns, ns_exp, patch_size)
    ...
    map_anno['ped_crossing'] = extract_local_ped_crossing(
        avm, ego_SE3_city, patch_box, patch_angle, patch_size, pt)
    ...
    map_anno['divider'] = extract_local_divider(
        nearby_dividers, ego_SE3_city, patch_box, patch_angle, patch_size, pt)

    return map_anno, pt, patch_box
...
def extract_local_divider(nearby_dividers, ego_SE3_city, patch_box, patch_angle, patch_size, pt):
    patch = NuScenesMapExplorer.get_patch_coord(patch_box, patch_angle)
    line_list = []
    ...
    lines = pt._one_type_line_geom_to_instances(line_list)
    results = []
    for line in lines:
        results.append(np.array(line.coords))
...
def extract_local_ped_crossing(avm, ego_SE3_city, patch_box, patch_angle, patch_size, pt):
    ped_list = []
    for pc in avm.get_scenario_ped_crossings():
        ped_list.append(pc.polygon)
    ...
    geom_dict = {"ped_crossing": polygon_list}
    ins_dict = pt.ped_poly_geoms_to_instances(geom_dict)
