...
import torch
from visual_copy import save_tensor
from mmcv.parallel import DataContainer as DC
import random

from nuscenes_dataset_copy import CustomNuScenesDataset
...
class LiDARInstanceLines(object):
    ...
    def fixed_num_sampled_points_ambiguity(self):
        ...
        for instance in self.instance_list:
            ...
            if instance.has_z:
                sampled_points = np.array([list(instance.interpolate(
                    distance).coords) for distance in distances]).reshape(-1, 3)
            else:
                sampled_points = np.array([list(instance.interpolate(
        ...
        instance_points_tensor[:, :, 2] = torch.clamp(
            instance_points_tensor[:, :, 2], min=self.min_z, max=self.max_z)
        instance_points_tensor = instance_points_tensor if is_3d else instance_points_tensor[
            :, :, :2]
    ...
    def shift_fixed_num_sampled_points(self):
        ...
        for fixed_num_pts in fixed_num_sampled_points:
            ...
            shift_pts[:, :, 2] = torch.clamp(
                shift_pts[:, :, 2], min=self.min_z, max=self.max_z)
    ...
    def shift_fixed_num_sampled_points_v1(self):
        ...
        for fixed_num_pts in fixed_num_sampled_points:
            ...
            if not is_poly:
                padding = torch.full(
                    [shift_num-shift_pts.shape[0], pts_num, shift_pts.shape[-1]], self.padding_value)
                shift_pts = torch.cat([shift_pts, padding], dim=0)
                # padding = np.zeros((self.num_samples - len(sampled_points), 2))
                # sampled_points = np.concatenate([sampled_points, padding], axis=0)
            instances_list.append(shift_pts)
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            ...
            if instance_label == 3:
                ...
                sampled_points = np.array([list(instance.interpolate(
                    distance).coords) for distance in distances]).reshape(-1, coords_num)
                ...
                if is_poly:
                    ...
                    for shift_right_i in range(shift_num):
                        shift_pts = np.roll(
                            pts_to_shift, shift_right_i, axis=0)
            ...
            if multi_shifts_pts_tensor.shape[0] < final_shift_num:
                padding = torch.full([final_shift_num-multi_shifts_pts_tensor.shape[0],
                                     self.fixed_num, multi_shifts_pts_tensor.shape[-1]], self.padding_value)
    ...
    def shift_fixed_num_sampled_points_v3(self):
        ...
        for instance in self.instance_list:
            ...
            if multi_shifts_pts_tensor.shape[0] < 2*final_shift_num:
                padding = torch.full([final_shift_num*2-multi_shifts_pts_tensor.shape[0],
                                     self.fixed_num, multi_shifts_pts_tensor.shape[-1]], self.padding_value)
                multi_shifts_pts_tensor = torch.cat(
                    [multi_shifts_pts_tensor, padding], dim=0)
            instances_list.append(multi_shifts_pts_tensor)
...
class VectorizedAV2LocalMap(object):
            ...
            if self.aux_seg['seg_classes'] == 1:
                ...
                if self.aux_seg['pv_seg']:
                    ...
                    gt_pv_semantic_mask = np.zeros(
                        (num_cam, 1, img_shape[0] // feat_down_sample, img_shape[1] // feat_down_sample), dtype=np.uint8)
                            ...
                            if self.aux_seg['pv_seg']:
                                for cam_index in range(num_cam):
                                    self.line_ego_to_pvmask(
                                        instance, gt_pv_semantic_mask[cam_index][0], lidar2feat[cam_index], color=1, thickness=self.aux_seg['pv_thickness'])
                        else:
                            print(instance.geom_type)
            else:
                if self.aux_seg['bev_seg']:
                    gt_semantic_mask = np.zeros(
                        (len(self.vec_classes), self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8)
                else:
                    gt_semantic_mask = None
                if self.aux_seg['pv_seg']:
                    num_cam = len(example['img_metas'].data['pad_shape'])
                    gt_pv_semantic_mask = np.zeros((num_cam, len(
                        self.vec_classes), img_shape[0] // feat_down_sample, img_shape[1] // feat_down_sample), dtype=np.uint8)
                    lidar2img = example['img_metas'].data['lidar2img']
                    scale_factor = np.eye(4)
                    scale_factor[0, 0] *= 1/32
                    scale_factor[1, 1] *= 1/32
                ...
                for instance, instance_type in vectors:
                    if instance_type != -1:
                        ...
                        if instance.geom_type == 'LineString':
                            if self.aux_seg['bev_seg']:
                                self.line_ego_to_mask(
                                    instance, gt_semantic_mask[instance_type], color=1, thickness=self.thickness)
                            if self.aux_seg['pv_seg']:
                                for cam_index in range(num_cam):
                                    self.line_ego_to_pvmask(
                                        instance, gt_pv_semantic_mask[cam_index][instance_type], lidar2feat[cam_index], color=1, thickness=self.aux_seg['pv_thickness'])
                        else:
                            print(instance.geom_type)
        else:
            for instance, instance_type in vectors:
    ...
    def line_ego_to_pvmask(self,
        ...
        cv2.polylines(mask, np.int32([pix_coords]),
                      False, color=color, thickness=thickness)
    ...
    def line_ego_to_mask(self,
        ...
        cv2.polylines(mask, np.int32([coords]),
                      False, color=color, thickness=thickness)
...
class CustomAV2OfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        # import pdb;pdb.set_trace()
        data_infos = list(
            sorted(data['samples'], key=lambda e: e['timestamp']))
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        if isinstance(anns_results['gt_vecs_pts_loc'], LiDARInstanceLines):
            ...
            try:
                gt_vecs_pts_loc = gt_vecs_pts_loc.flatten(
    ...
    def to_gt_vectors(self,
        ...
        for gt_instance, gt_label in zip(gt_instances, gt_labels):
            pts, pts_num = sample_pts_from_line(
                gt_instance, patch_size=self.patch_size)
            gt_vectors.append({
                'pts': pts,
                'pts_num': pts_num,
                'type': int(gt_label)
    ...
    def _evaluate_single(self,
        ...
        for metric in metrics:
            ...
            for i, name in enumerate(self.MAPCLASSES):
                ...
                detail['AV2Map_{}/{}_AP'.format(metric, name)
                       ] = cls_aps.mean(0)[i]
            ...
            for i, name in enumerate(self.MAPCLASSES):
                for j, thr in enumerate(thresholds):
                    if metric == 'chamfer':
                        detail['AV2Map_{}/{}_AP_thr_{}'.format(
...
def sample_pts_from_line(line,
    ...
    if normalize:
        sampled_points[:, :2] = sampled_points[:, :2] / \
            np.array([patch_size[1], patch_size[0]])
    ...
    if fixed_num < 0:
        if num_valid < num_samples:
            padding = np.zeros(
                (num_samples - len(sampled_points), sampled_points.shape[-1]))
            sampled_points = np.concatenate([sampled_points, padding], axis=0)
        else:
